...
import type { EventUpload, MemoryEvent, WizardFormData } from "@/types/event";
...
const EventDetail = () => {
  ...
  const { getEventById, updateEvent, deleteUpload, refreshEvents, events } =
    useEvents();
      ...
      if (giftData) {
        newUploads.push(
          ...giftData.map((g) => ({
            ...
            guestName: g.guest_name || "Guest",
            createdAt: g.created_at,
            isApproved: true,
            isAnonymous: false, // Field not in table yet, defaulting to false
          })),
        );
      }
  ...
  const totalGiftsAmount = gifts.reduce(
    (acc, curr) => acc + (curr.giftAmount || 0),
    0,
  );
  ...
  const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setEvent((prev) =>
        prev ? { ...prev, coverImage: publicUrlData.publicUrl } : null,
      );
      toast({
        title: "Cover updated!",
        description: "Your new cover image looks great.",
      });
    } catch (error) {
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div className="flex items-center gap-2 sm:ml-auto">
              ...
              {event.isLocked && (
                <Badge
                  variant="secondary"
                  className="gap-1 bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800"
                >
      ...
      <main className="container mx-auto px-4 py-8">
                      ...
                      {photos.length === 0 ? (
                        <div className="text-center py-12 bg-muted/50 rounded-lg">
                          ...
                          <p className="text-sm text-muted-foreground px-4">
                            Share the QR code with your guests to start
                            collecting memories
                          </p>
                        </div>
                      ) : (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {videos.map((video) => (
                            <div
                              ...
                              onClick={() =>
                                openLightbox(videos, videos.indexOf(video))
                              }
                            >
                              <video
                                src={video.content}
                ...
                },
                {
                  ...
                  label: (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="hidden xs:inline">
                        Gifts (₦{totalGiftsAmount.toLocaleString()})
                      </span>
                      <span className="xs:hidden">
                        (₦{totalGiftsAmount.toLocaleString()})
                      </span>
                    </div>
                  ),
                  content: (
                    <div className="mt-2">
                      {gifts.length === 0 ? (
                        <div className="text-center py-12 bg-muted/50 rounded-lg">
                          ...
                          <p className="text-sm text-muted-foreground px-4">
                            Enable gifting in settings to let guests contribute
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-4">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-1">
                                  <div className="flex flex-col">
                                    ...
                                    {gift.isAnonymous && (
                                      <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold bg-muted px-1.5 py-0.5 rounded w-fit mt-0.5">
                                        Anonymous
                                      </span>
                                    )}
                                  </div>
        ...
        <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
          <DialogContent
            className="sm:max-w-4xl p-0 overflow-hidden border-none bg-transparent shadow-none"
...
function ShareCard({ shareUrl, event, handleCopyLink }: ShareCardProps) {
  return (
    <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
      ...
      <div className="mb-4">
        ...
        <p className="text-xl font-mono font-bold tracking-[0.1em] text-foreground mt-1">
          {event.shareCode}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-border/50">
        <Button
          ...
          onClick={() =>
            window.open(
              `https://wa.me/?text=${encodeURIComponent(`Check out this event: ${shareUrl}`)}`,
              "_blank",
            )
          }
        >
        ...
        <Button
          ...
          onClick={() =>
            (window.location.href = `mailto:?subject=${encodeURIComponent(`You're invited: ${event.name}`)}&body=${encodeURIComponent(`Check out this event: ${shareUrl}`)}`)
          }
        >
...
function SettingsCard({ event, handleToggleSetting }: SettingsCardProps) {
  return (
    <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label htmlFor="live-feed" className="text-sm font-medium">
              Live Feed (Guest View)
            </Label>
            <p className="text-[11px] text-muted-foreground">
              Allow guests to see other uploads
            </p>
          </div>
          <Switch
            id="live-feed"
          ...
          <div className="flex items-center justify-between pl-6 border-l-2 border-border ml-1.5">
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-muted-foreground/80">
                Hide total amount from public page
              </p>
            </div>
            <Switch
              id="hide-total"
              ...
              onCheckedChange={(v) =>
                handleToggleSetting("isGiftTotalHidden", v)
              }
              disabled={event.isLocked}
            />
          </div>
...
function StatsCard({
  ...
  const photoRatio =
    total > 0
      ? (photos.length / (photos.length + videos.length || 1)) * 100
      : 0;
        ...
        <div className="flex-1 space-y-4 w-full">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-muted/30 rounded-lg p-2 text-center">
              <p className="text-[8px] text-muted-foreground uppercase font-black mb-1">
                Photos
              </p>
            ...
            <div className="bg-muted/30 rounded-lg p-2 text-center col-span-3 mt-1">
              ...
              <p className="font-bold text-sm">
                {uploads.filter((u) => u.type === "gift").length}
              </p>
            </div>
          </div>
        </div>
