...
export const useEvents = () => {
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          `
          *,
          media(id, file_type, file_url, uploaded_by, created_at, is_approved),
      ...
      return (data || []).map((e) => ({
        ...
        uploads: [
          ...
          ...(e.gifts || [])
            .filter((g: { status: string }) => g.status === "successful")
            .map(
              (g: {
                id: string;
                amount: number;
  ...
  const updateEventMutation = useMutation({
    mutationFn: async ({
      ...
      if (typeof updates.isLocked !== "undefined")
        dbUpdates.is_locked = updates.isLocked;
      if (typeof updates.isUploadsEnabled !== "undefined")
        dbUpdates.is_uploads_enabled = updates.isUploadsEnabled;
      if (typeof updates.isMessagesEnabled !== "undefined")
        dbUpdates.is_messages_enabled = updates.isMessagesEnabled;
      if (typeof updates.isGiftingEnabled !== "undefined")
        dbUpdates.is_gifting_enabled = updates.isGiftingEnabled;
      if (typeof updates.isLiveFeedEnabled !== "undefined")
        dbUpdates.is_live_feed_enabled = updates.isLiveFeedEnabled;
      if (typeof updates.isGiftTotalHidden !== "undefined")
        dbUpdates.is_gift_total_hidden = updates.isGiftTotalHidden;
      if (updates.groomFirstName)
        dbUpdates.groom_first_name = updates.groomFirstName;
      if (updates.groomLastName)
        dbUpdates.groom_last_name = updates.groomLastName;
      if (updates.brideFirstName)
        dbUpdates.bride_first_name = updates.brideFirstName;
      if (updates.brideLastName)
        dbUpdates.bride_last_name = updates.brideLastName;
      if (updates.religiousRiteVenue)
        dbUpdates.religious_rite_venue = updates.religiousRiteVenue;
      if (updates.religiousRiteStartTime)
        dbUpdates.religious_rite_start_time = updates.religiousRiteStartTime;
      if (updates.religiousRiteEndTime)
        dbUpdates.religious_rite_end_time = updates.religiousRiteEndTime;
      if (updates.receptionVenue)
        dbUpdates.reception_venue = updates.receptionVenue;
      if (updates.receptionStartTime)
        dbUpdates.reception_start_time = updates.receptionStartTime;
      if (updates.receptionEndTime)
        dbUpdates.reception_end_time = updates.receptionEndTime;
      if (typeof updates.isLocationPublic !== "undefined")
        dbUpdates.is_location_public = updates.isLocationPublic;
  ...
  return {
    ...
    updateEvent: (id: string, updates: Partial<MemoryEvent>) =>
    ...
    deleteUpload: (uploadId: string) =>
      deleteUploadMutation.mutateAsync({ uploadId }),
    refreshEvents: () =>
      queryClient.invalidateQueries({ queryKey: ["events"] }),
  };
};
